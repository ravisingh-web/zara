...
class ApiKeys {
  ...
  static const List<Map<String, String>> gemModels = [
    ...
    {'id': 'gemini-2.5-flash-preview-tts',        'name': 'Gemini 2.5 Flash TTS',  'desc': 'Best TTS quality'},
  ...
  static const List<Map<String, String>> geminiTtsVoices = [
    {'id': 'Zephyr', 'name': 'Zephyr  — Warm female (default)'},
    {'id': 'Aoede',  'name': 'Aoede   — Soft female'},
    {'id': 'Kore',   'name': 'Kore    — Clear female'},
    {'id': 'Leda',   'name': 'Leda    — Bright female'},
    {'id': 'Charon', 'name': 'Charon  — Deep male'},
    {'id': 'Fenrir', 'name': 'Fenrir  — Strong male'},
    {'id': 'Puck',   'name': 'Puck    — Playful'},
  ];

  static const _defGem   = 'gemini-2.5-flash-preview-tts';
  static const _defVoice = 'rdz6GofVsYlLgQl2dBEE';
  ...
  static Future<void> init() async {
    ...
    if (kDebugMode) debugPrint('ApiKeys init — model:$_model EL:$_elEnabled');
  }
  ...
  static Future<bool> save({
    ...
    try {
      ...
      if (gemKey != null && gemKey.isNotEmpty) {
        ...
        _gemKey = gemKey; _prov = ApiProvider.gemini;
        ok = await _prefs.setString(_kGem, gemKey) && ok;
      }
      if (elKey != null) { _elKey = elKey; ok = await _prefs.setString(_kElKey, elKey) && ok; }
      if (elEnabled != null) { _elEnabled = elEnabled; ok = await _prefs.setBool(_kElEnable, elEnabled) && ok; }
      if (model != null && gemModels.any((m) => m['id'] == model)) {
        _model = model; ok = await _prefs.setString(_kModel, model) && ok;
      }
      ...
      return ok;
    } catch (e) { if (kDebugMode) debugPrint('ApiKeys.save: $e'); return false; }
  }
  ...
  static Map<String, dynamic> get status => {
    'configured': ready, 'provider': 'gemini', 'model': _model,
    'gemSet': _gemKey.isNotEmpty, 'elEnabled': _elEnabled, 'elSet': _elKey.isNotEmpty,
  };
  static String defaultModelFor(ApiProvider p) => _defGem;
  static bool isValidModel(String id, ApiProvider p) => gemModels.any((m) => m['id'] == id);
