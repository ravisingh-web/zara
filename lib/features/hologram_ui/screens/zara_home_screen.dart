...
// ✅ Plasma Orb • Status HUD • Response Panel • Voice Commands • Mood UI
// ✅ Provider Integration • Real-Time Updates • Glassmorphic Design

import 'dart:async';
import 'dart:math';  // ✅ Added for Random, sin, cos
...
import '../../zara_engine/models/zara_state.dart';  // ✅ Added ZaraState import
...
// Screen imports
import '../../../../screens/settings_screen.dart';  // ✅ Ensure correct path
...
/// Main holographic home screen for Z.A.R.A.
/// Features: Plasma orb, status HUD, response panel, voice commands, mood-reactive UI
...
class _ZaraHomeScreenState extends State<ZaraHomeScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Initialize pulse animation for orb breathing effect
    ...
    // Activate Z.A.R.A. on screen load
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        context.read<ZaraController>().activate();
      }
    });
  }
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // Central Plasma Orb (Interactive)
          Center(
          ...
          // Central Response Panel (Dialogue/Code/Alerts)
          ...
          Positioned(
            ...
            left: 0,
            right: 0,
            child: _buildCommandArea(zara, state),
          ),

          // Listening Animation Overlay
          if (_isListening) _buildListeningOverlay(state),

          // Full Holographic Overlay (Settings/Details)
          if (_isOverlayVisible) _buildHolographicOverlay(zara, state),
        ],
      ),

      // Handle hardware back button (fixed: use PopScope instead of deprecated onPopInvokedWithResult)
      // Note: onPopInvokedWithResult requires Flutter 3.22+, using simpler approach for compatibility
    );
  }
  ...
  Widget _buildBackgroundMesh(Mood mood) {
    return CustomPaint(
      size: Size.infinite,
      painter: _BackgroundMeshPainter(
        color: mood.primaryColor.withOpacity(0.06),
        time: _pulseAnimation.value,
      ),
    );
  }
  ...
  /// Bottom command input with voice button and floating prompts
  Widget _buildCommandArea(ZaraController zara, ZaraState state) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          // Floating suggestion chips (mood-aware)
          FloatingPrompts(
            mood: state.mood,
            onSelected: (prompt) => _handleCommand(prompt, zara),
          ),
  ...
  /// Text input with voice button and send action
  Widget _buildInputField(ZaraController zara, ZaraState state) {
    return Container(
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(20),
        border: Border.all(
          color: state.mood.primaryColor.withOpacity(0.6),
          width: 1.5,
        ),
        boxShadow: [
          BoxShadow(
            color: state.mood.primaryColor.withOpacity(0.2),
            blurRadius: 12,
            spreadRadius: 1,
          ),
        ],
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                hintStyle: AppTextStyles.terminalText.copyWith(
          ...
          IconButton(
            ...
            onPressed: () {
              if (_commandController.text.trim().isNotEmpty) {
                _handleCommand(_commandController.text.trim(), zara);
              }
            },
  ...
  /// Toggle voice input mode (simulated - integrate speech_to_text for real STT)
  void _toggleVoiceInput(ZaraController zara) {
    ...
    if (_isListening) {
      ...
      // TODO: Integrate real speech_to_text package here
      // For now, simulate voice input with timer
  ...
  /// Start simulated voice amplitude animation
  void _startAmplitudeSimulation() {
    ...
    _amplitudeTimer = Timer.periodic(const Duration(milliseconds: 100), (timer) {
      ...
      setState(() {
        ...
        _voiceAmplitude = 0.3 + (DateTime.now().millisecond / 1000) * 0.7;
      });
    });
  }

  /// Stop voice input and reset amplitude
  ...
  /// Handle orb tap — activate Z.A.R.A. and show response
  ...
  /// Handle orb long press — show quick action menu
        ...
        child: Column(
          ...
          children: [
            Text(
              ...
              style: AppTextStyles.sciFiTitle.copyWith(fontSize: 16),
            ),
  ...
  /// Handle orb drag — adjust mood based on drag direction
  void _handleOrbDrag(DragUpdateDetails details, ZaraController zara) {
    final delta = details.delta.dy;

    if (delta < -20) {
      // Drag up → More energetic mood
      if (zara.state.mood != Mood.excited) {
        zara.changeMood(Mood.excited);
      }
    } else if (delta > 20) {
      // Drag down → Calmer mood
      if (zara.state.mood != Mood.calm) {
        zara.changeMood(Mood.calm);
      }
    }
  }

  /// Quick action button for orb menu
  ...
  /// Handle user command (text or voice)
  void _handleCommand(String command, ZaraController zara) {
    if (command.isEmpty) return;

    final lower = command.toLowerCase();

    // ✅ Fixed: Use explicit string methods instead of ambiguous extension
    if (_containsAny(lower, ['guardian', 'security'])) {
      zara.toggleGuardianMode();
    } else if (_containsAny(lower, ['location', 'where'])) {
      _getCurrentLocation();
    } else if (_containsAny(lower, ['photo', 'camera'])) {
      _captureIntruderPhoto();
    ...
    // Add affection for positive interactions
    if (_containsAny(lower, ['thank', 'nice', 'good', 'love', 'pyar'])) {
      zara.addAffection();
    }
  }

  /// Helper: Check if string contains any keyword (avoids extension ambiguity)
  ...
  Future<void> _getCurrentLocation() async {
    ...
    if (location != null && mounted) {
      ...
      // ✅ Fixed: Use public generateResponse method
      zara.generateResponse(
        '📍 Location Found!\n'
  ...
  /// Capture intruder photo (Guardian Mode)
  Future<void> _captureIntruderPhoto() async {
    ...
    if (photoPath != null && mounted) {
      ...
      // TODO: Offer to send email with photo
      // await EmailService().sendIntruderAlert(photoPath: photoPath);
    } else {
  ...
  Widget _buildListeningOverlay(ZaraState state) {
    return Container(
          ...
          children: [
            ...
            AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: state.mood.primaryColor,
                  width: 2 + _voiceAmplitude * 4,
                ),
                boxShadow: [
                  BoxShadow(
                    color: state.mood.primaryColor.withOpacity(0.6),
                    blurRadius: 20 + _voiceAmplitude * 40,
                    spreadRadius: 5,
                  ),
                ],
              ),
  ...
  // ========== Holographic Overlay (Full UI) ==========

  /// Full-screen holographic control panel
  Widget _buildHolographicOverlay(ZaraController zara, ZaraState state) {
    return Container(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  // Security Alerts (using dynamic Map to avoid type issues)
                  if (state.securityAlerts.isNotEmpty)
                    _buildSecurityAlertsCard(state.securityAlerts),
                ],
              ),
            ),
          ),
  ...
  Widget _buildMetricCard({
    ...
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(
          title,
          style: const TextStyle(color: AppColors.textDim, fontSize: 10, fontWeight: FontWeight.w500, fontFamily: 'RobotoMono'),  // ✅ Fixed: labelSmall now exists in AppTextStyles
        ),
        subtitle: Text(
          subtitle,
          style: AppTextStyles.baseTheme.labelSmall?.copyWith ?? AppTextStyles.baseTheme.labelSmall(
  ...
  Widget _buildGuardianCard(ZaraController zara, ZaraState state) {
    return Card(
      color: state.isGuardianActive
      ...
      shape: RoundedRectangleBorder(
        ...
        side: BorderSide(
          color: state.isGuardianActive
              ? AppColors.errorRed
              : AppColors.textDim,
          width: state.isGuardianActive ? 1.5 : 0.5,
        ),
      ),
      child: ListTile(
        leading: Icon(
        ...
        title: const Text(
          'Guardian Mode',
          style: const TextStyle(color: AppColors.textDim, fontSize: 10, fontWeight: FontWeight.w500, fontFamily: 'RobotoMono'),
        ),
        subtitle: Text(
          state.isGuardianActive
              ? 'ACTIVE • Monitoring'
              : 'Standby • Tap to activate',
          style: AppTextStyles.baseTheme.labelSmall?.copyWith ?? AppTextStyles.baseTheme.labelSmall(
            color: state.isGuardianActive
                ? AppColors.errorRed
                : AppColors.textDim,
  ...
  /// Security alerts card (using dynamic Map for compatibility)
  Widget _buildSecurityAlertsCard(List<dynamic> alerts) {
    return Card(
        ...
        child: Column(
          ...
          children: [
            ...
            ...alerts.take(3).map((alert) {
              final msg = (alert as Map<String, dynamic>)['message'] ?? 'Alert';
              return Padding(
                padding: const EdgeInsets.only(bottom: 6),
                child: Text(
                  '• $msg',
                  style: AppTextStyles.terminalText.copyWith(fontSize: 11),
                ),
              );
            }).toList(),
          ],
...
class _BackgroundMeshPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final random = Random((time * 400).toInt());  // ✅ sin/cos/Random now work with dart:math
  ...
  @override
  bool shouldRepaint(_BackgroundMeshPainter old) {
